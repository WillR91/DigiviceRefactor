...
AdventureState::AdventureState(Game* game) :
    ...
    currentNode_(nullptr) // Initialize currentNode_ to nullptr
{
    ...
    // Store the current node data pointer
    currentNode_ = &nodeData;
...
void AdventureState::enter() {
    ...
    // Update the current node data pointer
    const Digivice::NodeData& nodeData = playerData->getCurrentNodeData();
    currentNode_ = &nodeData;
...
void AdventureState::render(PCDisplay& display) {
    // Use our new rendering methods to handle background and character
    render_background_layers(display);
    render_character(display);
    
    // Render fade_to_battle overlay if active (keep this part from the original)
    if (is_fading_to_battle_ && battle_fade_alpha_ > 0.0f) {
        ...
        SDL_Rect fullscreen_rect = {0, 0, display.getWidth(), display.getHeight()};
...
void AdventureState::render_background_layers(PCDisplay& display) {
    // Even if we don't have a currentNode, we'll still try to render the
    // background textures directly for backward compatibility
    
    // Get display dimensions
    int screenWidth = display.getWidth();
    int screenHeight = display.getHeight();

    // For testing - render the background textures directly
    // Later we'll refactor to use the node data properly
    if (bgTexture0_) {
        int texW, texH;
        SDL_QueryTexture(bgTexture0_, nullptr, nullptr, &texW, &texH);
        
        // Position to center the background vertically in the middle of screen
        int posY = (screenHeight - texH) / 2;
        
        // Create destination rectangle for centered rendering
        SDL_Rect destRect = { (screenWidth - texW) / 2, posY, texW, texH };
        
        // Draw the texture
        display.drawTexture(bgTexture0_, nullptr, &destRect);
    }
    
    // Same for other background layers
    if (bgTexture1_) {
        int texW, texH;
        SDL_QueryTexture(bgTexture1_, nullptr, nullptr, &texW, &texH);
        int posY = (screenHeight - texH) / 2;
        SDL_Rect destRect = { (screenWidth - texW) / 2, posY, texW, texH };
        display.drawTexture(bgTexture1_, nullptr, &destRect);
    }
    ...
    if (bgTexture2_) {
        ...
        int posY = (screenHeight - texH) / 2;
        SDL_Rect destRect = { (screenWidth - texW) / 2, posY, texW, texH };
        display.drawTexture(bgTexture2_, nullptr, &destRect);
    }
}

void AdventureState::render_character(PCDisplay& display) {
    if (!current_partner_definition_) return;
    ...
    SDL_Texture* currentFrame = partnerAnimator_.getCurrentFrame();
    if (!currentFrame) return;
    
    // Get frame dimensions
    int frameWidth, frameHeight;
    SDL_QueryTexture(currentFrame, NULL, NULL, &frameWidth, &frameHeight);

    // Calculate ground position from bottom of screen
    // For now, use a fixed position that looks good with the background
    int groundY = screenHeight / 2 + 30; // Adjust this value to match the ground in your background
    
    // Position character at center horizontally, on the ground
    int characterX = screenWidth / 2 - (frameWidth / 2);
    int characterY = groundY - frameHeight;
    
    // Create destination rectangle
    SDL_Rect destRect = { characterX, characterY, frameWidth, frameHeight };
    
    // Draw the character
    display.drawTexture(currentFrame, NULL, &destRect);
}
