...
// GameState.h is included via Game.h
#include <SDL_log.h>
#include <stdexcept>
...
bool Game::init(const std::string& title, int width, int height) {
    ...
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_TIMER) < 0) { /* Log error */ return false; }
    if (!SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "0")) { /* Log warn */ }
    ...
    if (!display.init(title.c_str(), width, height)) { /* Log error */ SDL_Quit(); return false; }
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "PCDisplay Initialized.");

    if (!assetManager.init(display.getRenderer())) { /* Log error */ display.close(); SDL_Quit(); return false; }
    ...
    } catch (const std::filesystem::filesystem_error& e) { /* Log error */ }
    // --- END CWD LOGGING ---
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Attempting to load initial assets...");
     ...
     // --- END Paths ---
     if (!assets_ok) { /* Log error, cleanup, return false */ return false; }
    ...
    } catch (const std::exception& e) { /* Log error, cleanup, return false */ return false; }
...
// --- Run Loop ---
void Game::run() {
    if (!is_running || states_.empty()) { /* Log error */ return; }
    ...
    while (is_running) {
        ...
        if (!states_.empty()) {
            ...
            if (currentStatePtr) {
                SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "RunLoop: Updating state %p", currentStatePtr);
                currentStatePtr->handle_input();
                currentStatePtr->update(delta_time);
            } else {
                 // Should technically be caught by the empty check, but safety first
                 SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "RunLoop Update: Top state pointer is NULL despite non-empty stack!");
                 is_running = false; // Treat as error
            }
        } else {
             // Should also not be reached due to loop condition, but for clarity
             SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "RunLoop Update: State stack unexpectedly empty.");
             is_running = false;
        }

        // --- Apply Pending State Changes --- <<< MOVED/ADDED >>>
        ...
        SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "RunLoop: Preparing to render.");
        if (!states_.empty()) { // Check stack again AFTER applying changes
             GameState* currentStateForRender = getCurrentState(); // <<< Calls logged version >>>
             SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "RunLoop: Got currentStateForRender = %p.", currentStateForRender);

             if (currentStateForRender) {
                 SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "RunLoop: Calling render() for state %p...", currentStateForRender);
                 display.clear(0x0000);
                 currentStateForRender->render(); // <<< CRASH POINT IF POINTER IS BAD >>>
                 display.present();
                 SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "RunLoop: Finished render() for state %p.", currentStateForRender);
             } else {
                  // This would be very strange if states_ is not empty but getCurrentState returns null
                  SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "RunLoop: Render phase - getCurrentState returned NULL despite non-empty stack?");
             }
        } else {
            // If stack became empty AFTER changes, quit gracefully
             SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,"RunLoop: Render phase - State stack empty, skipping render.");
             is_running = false; // Ensure loop exits if stack is now empty
        }

        // --- Check Running Flag ---
        if (!is_running) {
            SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "RunLoop: is_running is false, breaking loop.");
            break;
        }
...
void Game::pop_state() {
    if (!states_.empty()) {
         GameState* stateToPop = states_.back().get(); // Get pointer BEFORE popping
         SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Executing pop_state for state %p...", stateToPop);
         states_.pop_back(); // Destroys unique_ptr, calls destructor
...
void Game::applyStateChanges() {
    ...
    // Pop must happen before push to allow replacing states correctly
    if (request_pop_) {
        SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "ApplyStateChanges: Applying Pop request.");
        pop_state(); // Calls internal pop_state with logging
        request_pop_ = false;
    }
    if (request_push_) {
        SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "ApplyStateChanges: Applying Push request for state %p.", request_push_.get());
        push_state(std::move(request_push_)); // Calls internal push_state with logging
        request_push_.reset(); // Clear the request holder
    }
...
GameState* Game::getCurrentState() {
    ...
    // Log frequently, might want to disable later or use lower log level
    // SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "getCurrentState called. Stack size = %zu. Returning state = %p", states_.size(), topState);
    return topState;
}
// <<< END Added Logging >>>
